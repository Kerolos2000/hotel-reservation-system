  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 md:p-6 space-y-4">
      <div className="flex items-start justify-between gap-2">
        <div>
          <h3 className="text-lg md:text-xl font-bold text-gray-900">{reservation.room?.type} Room</h3>
          <p className="text-gray-600 text-xs md:text-sm mt-1">{reservation.room?.description}</p>
        </div>
        <span className="bg-green-100 text-green-800 text-xs font-medium px-2 md:px-3 py-1 rounded-full whitespace-nowrap">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 py-3 md:py-4 border-y border-gray-200">
        <div>
          ...
          <p className="font-semibold text-gray-900 text-sm md:text-base">{formatDate(reservation.checkInDate)}</p>
        </div>
        <div>
          <p className="text-gray-600 text-xs md:text-sm">Check-out</p>
          <p className="font-semibold text-gray-900 text-sm md:text-base">{formatDate(reservation.checkOutDate)}</p>
        </div>
        <div>
          <p className="text-gray-600 text-xs md:text-sm">Guests</p>
          <p className="font-semibold text-gray-900 text-sm md:text-base">{reservation.room?.capacity}</p>
        </div>
        <div>
          <p className="text-gray-600 text-xs md:text-sm">Total Price</p>
          <p className="font-semibold text-blue-600 text-sm md:text-base">${reservation.totalPrice}</p>
        </div>
      </div>
